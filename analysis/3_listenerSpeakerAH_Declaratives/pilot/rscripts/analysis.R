...
source("../../../helpers.R")
df.data <- read.csv("../../../../data/3_listenerSpeakerAH_Declaratives/pilot/3_listenerSpeakerAH_Declaratives_pilot-trials.csv", header=TRUE)
...
# pilot only, since we don't have enough datapoints
...
## lmer
# speaker
speaker_model <- lmer(speaker_response ~ predicate * centered_prior_rating * centered_embedded_content + (1 | workerid) + (1 | item), # only include random intercept since not enough observations 
                      analysis_data)
summary(speaker_model)
# ah
ah_model <- lmer(ah_response ~ predicate * centered_prior_rating * centered_embedded_content + (1 | workerid) + (1 | item), # only include random intercept since not enough observations 
                 analysis_data)
summary(ah_model)
...
## Bayesian, brm
# speaker
speaker_bayesian <- brm(speaker_response ~ predicate * centered_prior_rating * centered_embedded_content + (1 | workerid) + (1 | item), # only include random intercept since not enough observations 
                        analysis_data,
                        control=list(max_treedepth = 15, adapt_delta = 0.99),
                        file="../cache/brm_speaker")
summary(speaker_bayesian)

full_speaker_bayesian <- brm(speaker_response ~ predicate * centered_prior_rating * centered_embedded_content + (1 | workerid) + (1 | item), # only include random intercept since not enough observations 
                               full_analysis_data,
                               control=list(max_treedepth = 15, adapt_delta = 0.99),
                               file="../cache/full_brm_speaker")
summary(full_speaker_bayesian)

# ah
ah_bayesian <- brm(ah_response ~ predicate * centered_prior_rating * centered_embedded_content + (1 | workerid) + (1 | item), # only include random intercept since not enough observations 
                        analysis_data |>
...
full_ah_bayesian <- brm(ah_response ~ predicate * centered_prior_rating * centered_embedded_content + (1 | workerid) + (1 | item), # only include random intercept since not enough observations 
                             full_analysis_data |>
                               filter(predicate != "Polar"),
                             control=list(max_treedepth = 15, adapt_delta = 0.99),
